...
void printList()
{
   ...
   while(ptr != NULL)
	{        
	  printf("%s %s %s %s \n",ptr->id ,ptr->username,ptr->password,ptr->nickname);
	  ptr = ptr->next;
	}
}

//chen link tai vi tri dau tien
void insertFirst(char *id,char *username, char *password,char*nickname, int numberOfGame,int numberOfDraw,int numberOfWin,int numberOfLoss,int rank)
{
   //tao mot link
